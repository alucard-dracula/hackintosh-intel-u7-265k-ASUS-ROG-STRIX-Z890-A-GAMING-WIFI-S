...
With this version of the BIOS, when using integrated graphics for installation, the process may get stuck at the -v log. At this time, please turn on the graphics dock power (you must do this at this moment; do not turn on the graphics dock power at startup, or plug/unplug the Thunderbolt cable while the graphics dock is on), and switch the display to the graphics dock's display port to confirm whether the installation interface is displayed correctly. If it is displayed correctly, you can proceed as normal. You need to follow this step for every reboot during the installation process until the login screen appears.
