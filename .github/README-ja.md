...
このバージョンのBIOSでは、内蔵グラフィックスでインストールする際に -v ログで止まることがあります。この場合は、グラフィックドックの電源を入れてください（必ずこのタイミングで入れる必要があり、起動時にグラフィックドックの電源を入れたり、電源を入れたまま雷電ケーブルを抜き差ししたりしてはいけません）。
そして、ディスプレイをグラフィックドックの表示ポートに切り替え、インストール画面が正しく表示されているか確認してください。正しく表示されていれば、そのまま操作できます。インストール中の毎回の再起動でも、この手順を繰り返し、インストールが完了してログイン画面が表示されるまで続けてください。
