...
**特記事項**
この構成でインストールする場合、マザーボードのBIOSはできれば2302のバージョンがよく、3103のバージョンは少し厄介です。必ず最後の説明を確認してください。
...
1. 必須設定:
    ...
    1.3. X2APCIを無効にする（無効にしないとOpenCoreは起動できません。無効にした後、VT-dを有効に設定できますが、OpenCoreでDisableIoMapperを設定する必要があります）
...
ROG-STRIX-Z890-A-GAMING-WIFI-S-ASUS-3103 に関する補足説明:
このバージョンのBIOSでは、内蔵グラフィックスでインストールする際に -v ログが止まる現象が発生します。この場合、グラフィックドックの電源を入れてください（必ずこのタイミングで電源を入れ、起動時にグラフィックドックの電源を入れないでください。グラフィックドックの電源を入れた状態で Thunderbolt ケーブルを抜き差ししてはいけません）。
そして、ディスプレイをグラフィックドックの表示ポートに切り替え、インストール画面が正しく表示されているか確認してください。正しく表示されていれば、通常通り操作できます。インストール中の再起動のたびに、この手順を繰り返し、インストール完了後にログイン画面が表示されるまで行ってください。
